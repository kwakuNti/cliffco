...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($_POST["password"]) || empty($dob) || empty($mobile_number) || empty($country)) {
        ...
        $sql_check_email = "SELECT pid FROM People WHERE email=?";
    ...
    if (empty($errors)) {
        ...
        $sql_insert_user = "INSERT INTO People (first_name, last_name, email, password, date_of_birth, mobile_number, country)
                            VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt_insert_user = mysqli_prepare($conn, $sql_insert_user);
        mysqli_stmt_bind_param($stmt_insert_user, "sssssss", $first_name, $last_name, $email, $password, $dob, $mobile_number, $country);
